...
def plot_fft_diffs(diff_type, launch_data_dict, launch_colour_dict, show = False):
    figure, axis = plt.subplots(2, 1, figsize = (7, 10))
    for launch, data in launch_data_dict.items():
        colour = launch_colour_dict[launch]
        freqs, psd = signal.welch(data[diff_type], fs=1, nperseg=4096)
    ...
    if diff_type == 'l_diffs':
        figure.suptitle(r'$\Delta$ L: Power Spectral Density')
    elif diff_type == 'cart_pos_diffs':
        figure.suptitle(r'$\Delta$ 3D: Power Spectral Density')
    elif diff_type == 'c_diffs':
        figure.suptitle(r'$\Delta$ C: Power Spectral Density')
    elif diff_type == 'h_diffs':
        figure.suptitle(r'$\Delta$ H: Power Spectral Density')

    figure.text(0, 0.5, 'Power Spectral Density (dB)', va='center', rotation='vertical')
    figure.subplots_adjust(hspace=0.5)
    plt.tight_layout()

    plt.savefig('output/plots/Fourier_analysis/fft_diffs_' + diff_type + '.png', dpi=300)
    if show == True:
        plt.show()

def run_full_fouier_analysis(diff_types, constellations, launch_data_dict, launch_colour_dict):
    for diff_type in diff_types:
        for constellation in constellations:
            ...
            plot_fft_diffs(diff_type, constellation_data, constellation_colour)
