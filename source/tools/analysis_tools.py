...
def TLE_pair_analyse(pair_TLE_list: List[List[str]], plot: bool = False) -> Tuple[List[Any], List[Any], List[Any], List[Any], List[Any], List[Any], List[Any]]:
    ...
    ----------
    pair_TLE_list: list
        a list of two lists containing TLEs for the two spacecraft to be compared. TLE list can be generated by the read_TLEs() function. 
    plot: bool, optional
        Will plot the outputs below into various subplots. Defaults to False.
    savepath: str, optional
        Path in which to save said plots. Defaults to './TLE_analysis_results/'.

    Returns
    -------
    master_ephs: list
        tuple the list of ephemerides
    eph_alts: list 
        a list of altitudes for each spacecraft at each step
    h_diffs: list
        a list of height differences between each spacecraft at each step
    c_diffs: list
        a list of cross-track differences between each spacecraft at each step
    l_diffs: list
        a list of along-track differences between each spacecraft at each step
    cart_pos_diffs: list
        a list of 3D cartesian position differences between each spacecraft at each step
    times: list
        a list of times for each step 
    """
...
def analyze_NORAD_vs_SUP(args: Tuple[str, str, str, str, str]) -> None:
    """
    Analyze files containing NORAD Two-Line Element (TLE) data and supplemental (SUP) TLE data.
...
def calculate_stats(df_list: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    -------
    pandas.DataFrame
        Pandas dataframe containing statistics for each launch and each constellation.
    """
...
def launch_specific_stats(list_of_dfs: List[pd.DataFrame], export: bool = True) -> pd.DataFrame:
    ...
    list_of_dfs : list of pandas.DataFrame
        List of dataframes containing positional data.
    export : bool, optional
        Flag indicating whether to export the summary statistics to a CSV file. Defaults to True.

    Returns
    -------
    pandas.DataFrame
        Pandas dataframe containing summary statistics for each launch and each constellation.
    """
...
def compute_fft(df: pd.DataFrame, diff_type: str) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute the Fourier Transform (Fast Fourier Transfrom) on given data.

    Parameters
    ----------
    df : pandas.DataFrame
        The data frame containing the data on which FFT is to be computed.
    diff_type : str
        The type of difference for which FFT is to be computed.
    ...
    fftfreqs = sp.fftpack.fftfreq(len(diff_psd), 1./(96)) #there are 96 15-minute intervals in a day so this gives us the frequencies in days^-1 if our ephemeris is in 15-minute intervals. TODO: this needs to be un-hardcoded
    im = fftfreqs >0
...
def find_files_sup_gp_op(folder_path: str = 'external/ephem_TLE_compare') -> Tuple[List[str], List[str], List[str]]:
    ...
    tuple of list of str
        Lists of file paths for SUP TLEs, GP TLEs, and operator ephemerides, respectively.
    """
